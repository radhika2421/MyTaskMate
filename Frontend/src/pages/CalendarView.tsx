  ...
  return (
    <div className="grid gap-7">
      <section className="flex flex-wrap items-end justify-between gap-4">
        <div>
          <h2 className="text-3xl font-black text-slate-950 dark:text-white">Integrated Calendar</h2>
        ...
        <div className="flex flex-wrap gap-3">
          ...
          <a href={`${API_URL}/api/auth/google`} className="rounded-lg bg-white px-5 py-3 font-black text-slate-700 shadow-sm ring-1 ring-violet-100 dark:bg-slate-900 dark:text-slate-200 dark:ring-slate-800">
      ...
      <section className="grid gap-6 xl:grid-cols-[1.2fr_0.8fr]">
        <article className="rounded-lg border border-violet-100 bg-white/80 p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
          <div className="flex items-center justify-between">
            ...
            <span className="text-sm font-black text-violet-500">Google synced</span>
          </div>
          <div className="mt-6 grid grid-cols-7 gap-2 text-center">
            {['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map((day) => <span key={day} className="py-2 text-xs font-black uppercase text-slate-500">{day}</span>)}
            {monthCells.map((date, index) => {
              const isToday = date ? dateKey(date) === dateKey(new Date()) : false
              const hasEvent = date ? eventDateKeys.has(dateKey(date)) : false
              return (
                <div key={date ? date.toISOString() : `empty-${index}`} className={`relative grid aspect-square place-items-center rounded-lg border text-sm font-black ${!date ? 'border-transparent' : isToday ? 'border-violet-500 bg-violet-500 text-white shadow-lg shadow-violet-200 dark:shadow-none' : 'border-violet-50 bg-[#fbf7f2] text-slate-700 dark:border-slate-800 dark:bg-slate-950 dark:text-slate-200'}`}>
                  {date?.getDate()}
                  {hasEvent && <span className={`absolute bottom-2 h-1.5 w-1.5 rounded-full ${isToday ? 'bg-white' : 'bg-rose-500'}`} />}
                </div>
              )
            })}
          </div>
      ...
      {eventModalOpen && (
        <div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/60 p-4">
          <form onSubmit={submitEvent} className="w-full max-w-xl rounded-lg bg-white p-6 shadow-2xl dark:bg-slate-950">
            ...
            <div className="mt-6 flex justify-end gap-3"><button type="button" onClick={() => setEventModalOpen(false)} className="rounded-lg border border-slate-200 px-5 py-3 font-bold dark:border-slate-800">Cancel</button><button type="submit" disabled={saving} className="rounded-lg bg-violet-600 px-5 py-3 font-black text-white disabled:opacity-60">{saving ? 'Adding...' : 'Add to Google Calendar'}</button></div>
          </form>
        </div>
      )}
    </div>
